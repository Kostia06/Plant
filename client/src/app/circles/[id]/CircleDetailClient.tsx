...
export default function CircleDetailClient({ id }: { id: string }) {
    ...
    async function loadCircle() {
        ...
        const { data: circleData } = await supabase.from("circles").select("*").eq("id", id).single();
        ...
        const { data: memberRows } = await supabase.from("circle_members").select("user_id").eq("circle_id", id);
    ...
    async function loadFriendOptions() {
        ...
        const availableIds = friendIds.filter((friendId) => !existingIds.has(friendId));

        if (availableIds.length > 0) {
            const { data: profiles } = await supabase.from("profiles").select("id, display_name").in("id", availableIds);
    ...
    async function addMember(friendId: string) {
        await supabase.from("circle_members").insert({ circle_id: id, user_id: friendId });
    ...
    return (
        <div className="page-container ci-page">
            <div className="ci-detail-header">
                ...
                <span className="ci-detail-count">👥 {members.length} member{members.length !== 1 ? "s" : ""}</span>
            </div>
                    ...
                    <div className="lb-list">
                        {members.map((m, i) => {
                            ...
                            return (
                                ...
                                >
                                    <span className="lb-rank">{i < 3 ? RANK_BADGES[i] : `#${i + 1}`}</span>
                                    <div className="lb-avatar">
                                        {m.avatar_url ? (
                                            <img src={m.avatar_url} alt="" className="lb-avatar-img" />
                                        ) : (
                                            <span className="lb-avatar-fallback">{m.display_name[0]?.toUpperCase()}</span>
                                        )}
                                    </div>
                                    <div className="lb-info">
                                        ...
                                        <span className="lb-meta">
                                            {TREE_EMOJIS[m.tree_state] ?? "🌱"} {m.tree_state} · 🔥 {m.streak_days}d
                                        </span>
                                    </div>
            ...
            {isOwner && (
                <div className="ci-section">
                    {!showAddMember ? (
                        <button className="btn btn-primary ci-add-btn" onClick={loadFriendOptions}>
                            + Add Members
                        </button>
                    ) : (
                        <div className="ci-add-members">
                            ...
                            {friendOptions.length === 0 ? (
                                <p className="pf-empty">
                                    All your friends are already in this circle, or you have no friends yet
                                </p>
                            ) : (
                                    ...
                                    {friendOptions.map((f) => (
                                        <div key={f.id} className="fr-row">
                                            <div className="fr-avatar">
                                                <span className="fr-avatar-fallback">{f.display_name[0]?.toUpperCase()}</span>
                                            </div>
                                            <span className="fr-name">{f.display_name}</span>
                                            <button className="btn btn-sm btn-primary" onClick={() => addMember(f.id)}>
                                                + Add
                                            </button>
                                        </div>
                                    ))}
                            ...
                            <button className="btn btn-sm btn-outline ci-cancel-btn" onClick={() => setShowAddMember(false)}>
                                Cancel
                            </button>
                        </div>
                    )}
            ...
            <div className="ci-actions">
                {!isOwner && (
                    <button className="btn btn-outline ci-leave-btn" onClick={leaveCircle}>
                        Leave Circle
                    </button>
                )}
                {isOwner && (
                    <button className="btn btn-danger ci-delete-btn" onClick={deleteCircle}>
                        Delete Circle
                    </button>
                )}
            </div>
